...
export default function OrangeAlbumsView({ albums, members, search, onOpen, onCreate, onRename, onShare, onDelete, onSetCover, createRequestKey = 0, onCreateRequestHandled }) {
  ...
  const openShare = album => {
    ...
    setCanContribute(album.visibility === "family" ? album.allow_contributions === true : album.visibility === "selected" ? album.shared_can_contribute === true : false);
    setError("");
    setWarning("");
  };
  ...
  return <div className="od-orange-albums-view">
    ...
    <div className="od-orange-albums-view__grid">{visible.map(album => { const shareTitle=albumShareTitle(album);return <div className="od-orange-album-card" key={album.id} role="button" tabIndex="0" onClick={() => onOpen(album)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") onOpen(album); }}>
      <div className="od-orange-album-card__cover">{album.cover_thumbnail_url ? <img src={album.cover_thumbnail_url} alt="" /> : <span className="od-orange-album-card__placeholder">Álbum</span>}</div>
      <div className="od-orange-album-card__footer"><div className="od-orange-album-card__text"><strong>{album.title}</strong><small>{album.photo_count} elementos{shareTitle ? <span className={`od-orange-album-card__share od-orange-photo-card__share--${album.is_owner ? "owned" : "received"}`} title={shareTitle} aria-label={shareTitle}><IonIcon icon={peopleOutline} /></span> : null} · {album.is_owner ? album.visibility : "Compartido"}</small></div>
    ...
    {shareAlbum ? <div className="od-modal-backdrop"><section className="od-modal" role="dialog" aria-modal="true"><header className="od-modal-header"><h2 className="od-modal-title">Compartir álbum</h2><button className="od-modal-close" type="button" onClick={() => closeModal(setShareAlbum)}>×</button></header><div className="od-modal-body"><label className="od-form-field"><span className="od-form-label">Visibilidad</span><select className="od-filter-input" value={visibility} onChange={event => { const next=event.target.value;setVisibility(next);if(next==="private")setCanContribute(false); }}><option value="private">Solo yo</option><option value="family">Toda la familia</option><option value="selected">Miembros concretos</option></select></label>{visibility === "selected" ? <fieldset className="od-orange-photo-share__members"><legend>Miembros</legend>{members.map(member => <label key={member.id}><input type="checkbox" checked={selectedUserIds.includes(member.id)} onChange={() => setSelectedUserIds(current => current.includes(member.id) ? current.filter(id => id !== member.id) : [...current, member.id])} />{member.display_name}</label>)}</fieldset> : null}<label className="od-form-field"><span className="od-form-label">Permisos</span><select className="od-filter-input" value={canContribute ? "contribute" : "view"} disabled={visibility === "private"} onChange={event => setCanContribute(event.target.value === "contribute")}><option value="view">Solo pueden ver</option><option value="contribute">Pueden añadir fotos y vídeos</option></select><small>Permite añadir fotos y vídeos al álbum. No permite cambiar el nombre, compartirlo, elegir portada ni eliminar contenido de otras personas.</small></label>{warning ? <p className="od-inline-msg">{warning}</p> : null}{error ? <p className="od-status-line od-status-line--error">{error}</p> : null}<div className="od-modal-actions"><button className="od-btn od-btn-secondary" type="button" disabled={busy} onClick={() => closeModal(setShareAlbum)}>Cancelar</button><button className="od-btn od-btn-primary" type="button" disabled={busy || visibility === "selected" && !selectedUserIds.length} onClick={async () => { const response = await run(() => onShare(shareAlbum.id, { visibility, user_ids: visibility === "selected" ? selectedUserIds : [], can_contribute: visibility === "private" ? false : canContribute })); if (response) setShareAlbum(null); }}>Guardar</button></div></div></section></div> : null}
