...
async function trash(req,id,restore=false){const d=await detail(req,id,{allowTrash:true});if(!d.ok)return d;if(!d.payload.item.is_owner)return bad(403,"Solo el propietario puede gestionar la papelera.");const c=await pool.connect();try{await c.query('BEGIN');const locked=(await c.query(`SELECT id FROM public.orange_photos WHERE id=$1::uuid AND family_id=$2::uuid AND owner_user_id=$3::uuid FOR UPDATE`,[id,d.payload.auth.familyId,d.payload.auth.userId])).rows[0];if(!locked){await c.query('ROLLBACK');return bad(409,"PHOTO_RETAINED_BY_LIBRARY","La foto no puede cambiar de estado porque ya no está disponible.");}if(!restore&&await hasOtherLibraryMemberships(c,id,d.payload.auth.userId)){await c.query('ROLLBACK');return bad(409,"PHOTO_RETAINED_BY_LIBRARY","La foto pertenece a la biblioteca de otro usuario y todavía no puede moverse a la papelera.");}const r=(await c.query(`UPDATE public.orange_photos SET is_trashed=$3,trashed_at=CASE WHEN $3 THEN now() ELSE NULL END,updated_by=$2 WHERE id=$1 RETURNING *`,[id,d.payload.auth.userId,!restore])).rows[0];await recordPhotoEvent(c,{familyId:d.payload.auth.familyId,photoId:id,actorUserId:d.payload.auth.userId,eventType:restore?'restored':'moved_to_trash',...clientContext(req)});await c.query('COMMIT');return ok({item:r});}catch(error){await c.query('ROLLBACK');throw error;}finally{c.release();}}
async function addToLibrary(req,id){const auth=resolveAuthenticatedFamily(req);if(!auth.ok)return auth;if(!uuid(id))return bad(400,"Identificador no válido.");const client=await pool.connect();try{await client.query('BEGIN');const photo=(await client.query(`SELECT p.id,p.owner_user_id FROM public.orange_photos p WHERE p.family_id=$1::uuid AND p.id=$3::uuid AND p.is_trashed=false AND ${visibilitySql()} FOR UPDATE`,[auth.familyId,auth.userId,id])).rows[0];if(!photo){await client.query('ROLLBACK');return bad(404,"UPLOAD_NOT_FOUND","Foto no encontrada.");}const result=await client.query(`INSERT INTO public.orange_photo_library_items(photo_id,user_id) VALUES($1::uuid,$2::uuid) ON CONFLICT(photo_id,user_id) DO NOTHING RETURNING added_at`,[id,auth.userId]);const created=result.rowCount===1;await client.query('COMMIT');const refreshed=await detail(req,id);if(!refreshed.ok)return refreshed;return ok({item:refreshed.payload.item,created});}catch(error){await client.query('ROLLBACK');throw error;}finally{client.release();}}
async function removeFromLibrary(req,id){const auth=resolveAuthenticatedFamily(req);if(!auth.ok)return auth;if(!uuid(id))return bad(400,"Identificador no válido.");const client=await pool.connect();try{await client.query('BEGIN');const photo=(await client.query(`SELECT p.id,p.owner_user_id FROM public.orange_photos p JOIN public.orange_photo_library_items li ON li.photo_id=p.id AND li.user_id=$2::uuid WHERE p.family_id=$1::uuid AND p.id=$3::uuid FOR UPDATE OF p`,[auth.familyId,auth.userId,id])).rows[0];if(!photo){await client.query('ROLLBACK');return bad(404,"PHOTO_LIBRARY_ITEM_NOT_FOUND","La foto no pertenece a tu biblioteca.");}if(String(photo.owner_user_id)===String(auth.userId)){await client.query('ROLLBACK');return bad(409,"ORIGINAL_OWNER_LIBRARY_REQUIRED","El propietario original no puede quitar la foto de su biblioteca.");}await client.query(`DELETE FROM public.orange_photo_library_items WHERE photo_id=$1::uuid AND user_id=$2::uuid`,[id,auth.userId]);await client.query('COMMIT');return ok({id,removed:true});}catch(error){await client.query('ROLLBACK');throw error;}finally{client.release();}}
...
async function purge(req, id) {
  ...
  try{
    ...
    const locked=(await client.query(`SELECT id FROM public.orange_photos WHERE family_id=$1::uuid AND id=$2::uuid AND owner_user_id=$3::uuid AND is_trashed=true FOR UPDATE`,[auth.familyId,id,auth.userId])).rows[0];
    if(!locked){await client.query("ROLLBACK");return bad(409,"PHOTO_RETAINED_BY_LIBRARY","La foto no puede eliminarse definitivamente porque ya no está disponible.");}
    if(await hasOtherLibraryMemberships(client,id,auth.userId)){await client.query("ROLLBACK");return bad(409,"PHOTO_RETAINED_BY_LIBRARY","La foto pertenece a la biblioteca de otro usuario y no puede eliminarse definitivamente.");}
    const files = (await client.query(`SELECT id,bucket,object_key,variant,checksum_sha256 FROM public.orange_photo_files WHERE family_id=$1::uuid AND photo_id=$2::uuid ORDER BY CASE variant WHEN 'thumbnail' THEN 1 WHEN 'preview' THEN 2 WHEN 'poster' THEN 3 WHEN 'original' THEN 4 ELSE 5 END`, [auth.familyId, id])).rows;
...
async function updatePublicLink(req,id,type,regenerate=false){const auth=resolveAuthenticatedFamily(req);if(!auth.ok)return auth;if(!uuid(id))return bad(400,"Identificador no válido.");const table=type==='photo'?'orange_photos':'orange_photo_albums',archived=type==='photo'?'is_trashed=false':'is_archived=false';const current=(await pool.query(`SELECT * FROM public.${table} WHERE id=$1::uuid AND family_id=$2::uuid AND owner_user_id=$3::uuid AND ${archived}`,[id,auth.familyId,auth.userId])).rows[0];if(!current)return bad(404,type==='photo'?"Foto no encontrada.":"Álbum no encontrado.");if(current.public_enabled===true&&current.public_token&&!regenerate)return ok({public_link:publicLinkPayload(current,type)});const token=randomBytes(24).toString('base64url');const row=(await pool.query(`UPDATE public.${table} SET public_enabled=true,public_token=$1,public_created_at=now(),public_revoked_at=NULL,updated_by=$2::uuid WHERE id=$3::uuid RETURNING public_enabled,public_token,public_revoked_at`,[token,auth.userId,id])).rows[0];return ok({public_link:publicLinkPayload(row,type)});}
async function revokePublicLink(req,id,type){const auth=resolveAuthenticatedFamily(req);if(!auth.ok)return auth;if(!uuid(id))return bad(400,"Identificador no válido.");const table=type==='photo'?'orange_photos':'orange_photo_albums',archived=type==='photo'?'is_trashed=false':'is_archived=false';const row=(await pool.query(`UPDATE public.${table} SET public_enabled=false,public_revoked_at=now(),updated_by=$1::uuid WHERE id=$2::uuid AND family_id=$3::uuid AND owner_user_id=$1::uuid AND ${archived} RETURNING id`,[auth.userId,id,auth.familyId])).rows[0];return row?ok({public_link:{enabled:false}}):bad(404,type==='photo'?"Foto no encontrada.":"Álbum no encontrado.");}
...
async function publicPhotoByToken(token){const safe=publicToken(token);if(!safe)return bad(404,"PUBLIC_NOT_FOUND","Recurso público no encontrado.");const row=(await pool.query(`SELECT p.*,COALESCE(NULLIF(btrim(pe.preferred_name),''),btrim(concat_ws(' ',pe.first_name,pe.last_name)),'Usuario') owner_display_name FROM public.orange_photos p JOIN public.auth_users au ON au.id=p.owner_user_id LEFT JOIN public.persons pe ON pe.id=au.person_id WHERE p.public_enabled=true AND p.public_token=$1 AND p.public_revoked_at IS NULL AND p.is_trashed=false`,[safe])).rows[0];if(!row)return bad(404,"PUBLIC_NOT_FOUND","Recurso público no encontrado.");await signPhotoFiles([row]);return ok({item:publicPhoto(row),raw:row});}
...
async function publicAlbumByToken(token){const safe=publicToken(token);if(!safe)return bad(404,"PUBLIC_NOT_FOUND","Recurso público no encontrado.");const row=(await pool.query(`SELECT a.id,a.title,a.description,a.date_mode,a.date_start,a.date_end,a.cover_photo_id,COALESCE(NULLIF(btrim(p.preferred_name),''),btrim(concat_ws(' ',p.first_name,p.last_name)),'Usuario') owner_display_name,(SELECT count(*)::int FROM public.orange_photo_album_items ai JOIN public.orange_photos ph ON ph.id=ai.photo_id WHERE ai.album_id=a.id AND ph.is_trashed=false) photo_count FROM public.orange_photo_albums a JOIN public.auth_users au ON au.id=a.owner_user_id LEFT JOIN public.persons p ON p.id=au.person_id WHERE a.public_enabled=true AND a.public_token=$1 AND a.public_revoked_at IS NULL AND a.is_archived=false`,[safe])).rows[0];return row?ok({item:{...row,public_link_active:true}}):bad(404,"PUBLIC_NOT_FOUND","Recurso público no encontrado.");}
async function publicAlbumPhotos(token,query={}){const album=await publicAlbumByToken(token);if(!album.ok)return album;const page=Math.max(1,Number(query.page)||1),per=Math.min(100,Math.max(1,Number(query.per_page)||30)),values=[album.payload.item.id,per,(page-1)*per],base=`FROM public.orange_photo_album_items ai JOIN public.orange_photos p ON p.id=ai.photo_id JOIN public.auth_users au ON au.id=p.owner_user_id LEFT JOIN public.persons pe ON pe.id=au.person_id WHERE ai.album_id=$1::uuid AND p.is_trashed=false`;const total=Number((await pool.query(`SELECT count(*)::int total ${base}`,[album.payload.item.id])).rows[0].total);const rows=(await pool.query(`SELECT p.*,COALESCE(NULLIF(btrim(pe.preferred_name),''),btrim(concat_ws(' ',pe.first_name,pe.last_name)),'Usuario') owner_display_name ${base} ORDER BY p.captured_at DESC NULLS LAST,p.created_at DESC,p.id DESC LIMIT $2 OFFSET $3`,values)).rows;await signPhotoFiles(rows);return ok({items:rows.map(publicPhoto),page,per_page:per,total,has_more:page*per<total});}
async function publicAlbumPhoto(token,photoId){const album=await publicAlbumByToken(token);if(!album.ok)return album;if(!uuid(photoId))return bad(404,"PUBLIC_NOT_FOUND","Recurso público no encontrado.");const row=(await pool.query(`SELECT p.*,COALESCE(NULLIF(btrim(pe.preferred_name),''),btrim(concat_ws(' ',pe.first_name,pe.last_name)),'Usuario') owner_display_name FROM public.orange_photo_album_items ai JOIN public.orange_photos p ON p.id=ai.photo_id JOIN public.auth_users au ON au.id=p.owner_user_id LEFT JOIN public.persons pe ON pe.id=au.person_id WHERE ai.album_id=$1::uuid AND p.id=$2::uuid AND p.is_trashed=false`,[album.payload.item.id,photoId])).rows[0];if(!row)return bad(404,"PUBLIC_NOT_FOUND","Recurso público no encontrado.");await signPhotoFiles([row]);return ok({item:publicPhoto(row),raw:row});}
