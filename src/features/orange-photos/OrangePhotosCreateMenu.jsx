...
export default function OrangePhotosCreateMenu({ open, anchorRef, onClose, onCreateAlbum, onCreateCategory, onImport, canManageAlbums = true }) {
  if (!open) return null;
  return <OrangePhotosAnchoredPortal anchorRef={anchorRef} onClose={onClose} label="Cerrar menú">{style => <div className="od-action-menu od-orangephotos-create-menu" style={style} role="menu">{canManageAlbums ? <><p>Crear</p><button className="od-action-menu-item" type="button" role="menuitem" onClick={onCreateAlbum}><IonIcon icon={albumsOutline} />Crear álbum</button><button className="od-action-menu-item" type="button" role="menuitem" onClick={() => { onClose(); onCreateCategory(); }}><IonIcon icon={folderOutline} />Crear categoría</button><hr /></> : null}<p>Añadir fotos</p><button className="od-action-menu-item" type="button" role="menuitem" onClick={onImport}><IonIcon icon={cloudUploadOutline} />Importar fotos y vídeos</button></div>}</OrangePhotosAnchoredPortal>;
}
