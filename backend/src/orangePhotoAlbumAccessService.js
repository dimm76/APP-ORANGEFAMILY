...
async function syncAlbumRecipients(req, albumId, input, queryable = pool, options = {}) {
  ...
  try {
    ...
    const accessRevocationScope = options.preserveExternal === true ? " AND subject_type='family'" : "";
    await client.query(`UPDATE public.orange_photo_album_access SET status='revoked',revoked_at=now(),updated_at=now() WHERE album_id=$1::uuid AND status<>'revoked'${accessRevocationScope} AND NOT (user_id=ANY($2::uuid[]))`, [albumId, input.recipients.map((recipient) => recipient.user_id)]);
    ...
    await client.query(`DELETE FROM public.orange_photo_album_shares WHERE album_id=$1::uuid AND NOT (user_id=ANY($2::uuid[]))`, [albumId, familyIds]);
    if (familyIds.length) await client.query(`INSERT INTO public.orange_photo_album_shares(album_id,user_id,shared_by,can_contribute) SELECT $1::uuid,unnest($2::uuid[]),$3::uuid,$4 ON CONFLICT (album_id,user_id) DO UPDATE SET shared_by=EXCLUDED.shared_by,can_contribute=EXCLUDED.can_contribute`, [albumId, familyIds, auth.userId, input.allow_contributions]);
    await client.query(`UPDATE public.orange_photo_albums SET visibility=COALESCE($2::text,CASE WHEN cardinality($3::uuid[])=0 AND cardinality($4::uuid[])=0 THEN 'private' WHEN cardinality($4::uuid[])=0 AND cardinality($3::uuid[])=(SELECT count(*) FROM public.family_memberships membership JOIN public.auth_users family_user ON family_user.person_id=membership.person_id WHERE membership.family_id=$1::uuid AND membership.status='active' AND membership.role IN ('member','guest') AND COALESCE((membership.module_access ->> 'orange_photos')::boolean,false)=true AND family_user.status IN ('active','pending') AND family_user.id<>$5::uuid) THEN 'family' ELSE 'selected' END) WHERE id=$1::uuid`, [albumId, options.visibilityOverride == null ? null : String(options.visibilityOverride), familyIds, externalIds, auth.userId]);
    if (options.preserveExternal !== true) {
