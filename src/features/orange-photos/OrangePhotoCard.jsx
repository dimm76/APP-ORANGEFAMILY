...
function OrangePhotoCard({ photo, selectionMode, selected, onSelect, onOpen, eager = false }) {
  const [previewing, setPreviewing] = useState(false), [hoverDuration,setHoverDuration]=useState(null), [mediaCandidateIndex,setMediaCandidateIndex]=useState(0), [shouldLoad,setShouldLoad]=useState(eager), [mediaLoaded,setMediaLoaded]=useState(false), timerRef = useRef(null), videoRef = useRef(null), cardRef = useRef(null);
  ...
  const mediaCandidates=[...new Set((photo.media_type==="video"?[photo.poster_url,photo.thumbnail_url]:[photo.thumbnail_url,photo.preview_url,photo.original_url]).filter(Boolean))];
  ...
  }, [photo.id,photo.poster_url,photo.thumbnail_url,photo.preview_url,photo.original_url]);
  useEffect(() => {
    if (eager || shouldLoad) return undefined;
    const card=cardRef.current,scroller=card?.closest(".od-orange-photos__scroller");
    if (!card||!scroller||!("IntersectionObserver" in window)) {
      setShouldLoad(true);
      return undefined;
    }
    const observer=new IntersectionObserver(entries=>{if(!entries.some(entry=>entry.isIntersecting))return;setShouldLoad(true);observer.disconnect();},{root:scroller,rootMargin:"900px 0px",threshold:0});
