...
La pertenencia persistente puede actuar como fuente autenticada de acceso. `access_source` admite `owned`, `library`, `direct` y `album`, con prioridad `owned → library → direct → album`; `library` representa una membership de un usuario distinto del propietario original. La biblioteca principal todavía no cambia por defecto, y la papelera y Android tampoco cambian por defecto.

`POST /api/orange-photos/:id/library` permite crear de forma idempotente una membership cuando el usuario autenticado tiene acceso actual a la fotografía. Usa la clave primaria (`photo_id`, `user_id`), no duplica la fotografía ni sus archivos, conserva el acceso aunque desaparezca la compartición y mantiene inmutable `owner_user_id`. La protección de `trash`/`purge` de la Fase 3B continúa activa; esta operación todavía no cambia la biblioteca principal ni Android.
