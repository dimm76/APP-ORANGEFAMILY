...
async function signPhotoFiles(rows) {
  ...
  await Promise.all(rows.map(async row => { const variants = by.get(String(row.id)) || {},thumbnail=variants.thumbnail||variants.poster||variants.preview||variants.original,preview=variants.preview||variants.original;if(thumbnail){row.thumbnail_url=await getSignedOrangePhotoUrl(thumbnail);row.thumbnail_width=thumbnail.width;row.thumbnail_height=thumbnail.height;}if(preview)row.preview_url=await getSignedOrangePhotoUrl(preview);if(row.media_type === "video") { const poster = variants.poster || variants.thumbnail;if(variants.preview)row.video_preview_url=await getSignedOrangePhotoUrl(variants.preview);if(poster)row.poster_url=await getSignedOrangePhotoUrl(poster); }if(variants.original)row.original_url=await getSignedOrangePhotoUrl(variants.original);row.shared_user_ids=sharedBy.get(String(row.id))||[];row.albums=row.albums||[];row.tags=row.tags||[];}));
}
