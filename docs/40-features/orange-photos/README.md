...
La biblioteca web principal representa ahora `owned` + `library`: `owned` sigue significando propietario original y `library` una membership persistente de una foto cuyo propietario original es otro usuario. «Compartido conmigo» conserva `direct` + `album`; el visor web permite «Añadir a mi biblioteca» para una foto ajena accesible que todavía no tenga membership mediante el endpoint existente, sin cambiar `owner_user_id` ni duplicar archivos. El frontend de «Quitar de mi biblioteca» todavía no se implementa en esta subfase y Android continúa sin cambios. Las acciones de propietario en el visor y en la selección masiva se ocultan para elementos ajenos, manteniendo la selección, la descarga y las operaciones compatibles.
