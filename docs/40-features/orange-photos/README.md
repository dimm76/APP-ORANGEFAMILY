...
`POST /api/orange-photos/:id/library` permite crear de forma idempotente una membership cuando el usuario autenticado tiene acceso actual a la fotografía. Usa la clave primaria (`photo_id`, `user_id`), no duplica la fotografía ni sus archivos, conserva el acceso aunque desaparezca la compartición y mantiene inmutable `owner_user_id`. La protección de `trash`/`purge` de la Fase 3B continúa activa; esta membership ya incorpora la fotografía a la biblioteca web principal y Android sigue sin cambios.

La biblioteca web principal representa ahora `owned` + `library`: `owned` sigue significando propietario original y `library` una membership persistente de una foto cuyo propietario original es otro usuario. «Compartido conmigo» conserva `direct` + `album`; el visor web permite «Añadir a mi biblioteca» para una foto ajena accesible que todavía no tenga membership mediante el endpoint existente, sin cambiar `owner_user_id` ni duplicar archivos. Todavía no existe «Quitar de mi biblioteca» y Android continúa sin cambios. Las acciones de propietario en el visor y en la selección masiva se ocultan para elementos ajenos, manteniendo la selección, la descarga y las operaciones compatibles.
