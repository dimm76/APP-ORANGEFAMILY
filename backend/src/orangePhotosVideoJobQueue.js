...
async function enqueueVideoJob(photoId, { source, priority, options }) {
  const result = await pool.query(`INSERT INTO public.orange_photo_video_jobs (photo_id,family_id,source,priority,status,options_json,attempts,last_error,created_at,updated_at,started_at,finished_at) SELECT p.id,p.family_id,$2,$3,'pending',$4::jsonb,0,NULL,now(),now(),NULL,NULL FROM public.orange_photos p WHERE p.id=$1::uuid AND p.media_type='video' ON CONFLICT (photo_id) DO UPDATE SET source=CASE WHEN EXCLUDED.priority >= public.orange_photo_video_jobs.priority THEN EXCLUDED.source ELSE public.orange_photo_video_jobs.source END,priority=GREATEST(public.orange_photo_video_jobs.priority,EXCLUDED.priority),status=CASE WHEN public.orange_photo_video_jobs.status='processing' THEN 'processing' ELSE 'pending' END,options_json=CASE WHEN EXCLUDED.priority >= public.orange_photo_video_jobs.priority THEN EXCLUDED.options_json ELSE public.orange_photo_video_jobs.options_json END,last_error=NULL,updated_at=now(),started_at=CASE WHEN public.orange_photo_video_jobs.status='processing' THEN public.orange_photo_video_jobs.started_at ELSE NULL END,finished_at=NULL RETURNING photo_id,source,priority,status`, [photoId, source, priority, JSON.stringify(options || {})]);
