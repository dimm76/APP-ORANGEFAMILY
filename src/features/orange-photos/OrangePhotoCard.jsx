...
function OrangePhotoCard({ photo, selectionMode, selected, onSelect, onOpen, eager = false }) {
  const [previewing, setPreviewing] = useState(false), [hoverDuration,setHoverDuration]=useState(null), [failedImageUrl,setFailedImageUrl]=useState(null), timerRef = useRef(null), videoRef = useRef(null), cardRef = useRef(null);
  const label = photo.title?.trim() || photo.original_filename?.trim() || "Sin título";
  const poster = photo.poster_url || photo.thumbnail_url || null;
  ...
  const gridUrl = photo.media_type === "video" ? poster : photo.thumbnail_url || photo.preview_url || photo.original_url || null;
  const imageFailed = Boolean(gridUrl && failedImageUrl === gridUrl);
  const stop = () => {
