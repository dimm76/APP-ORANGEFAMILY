...
async function generateVideoPoster(req,id){const d=await detail(req,id,{allowTrash:true});if(!d.ok)return d;const photo=d.payload.item;if(!photo.is_owner)return bad(403,"Solo el propietario puede generar la miniatura.");if(photo.media_type!=="video")return bad(400,"La miniatura solo puede generarse para vídeos.");const variants=(await pool.query(`SELECT variant FROM public.orange_photo_files WHERE family_id=$1::uuid AND photo_id=$2::uuid AND variant=ANY($3::text[])`,[d.payload.auth.familyId,id,["original","poster","thumbnail"]])).rows.map(row=>row.variant);if(!variants.includes("original"))return bad(404,"El vídeo original no está disponible.");if(variants.includes("poster")||variants.includes("thumbnail"))return bad(409,"El vídeo ya dispone de miniatura.");await processStoredOrangePhotoVideo(id,{createPoster:true,createPreview:false,updateMetadata:false});return detail(req,id,{allowTrash:true});}
