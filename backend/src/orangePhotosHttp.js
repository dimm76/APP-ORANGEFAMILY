...
async function multipart(req, maxBytes) {
  ...
  if (!match) throw new Error("Content-Type multipart no válido.");
  const boundary = Buffer.from(`--${match[1] || match[2]}`), chunks = [];
  let total = 0;
  for await (const chunk of req) { total += chunk.length; if (total > maxBytes + 2 * 1024 * 1024 + 128 * 1024) throw new Error("Archivo demasiado grande."); chunks.push(chunk); }
  ...
  while (start < body.length) {
    ...
    if (filename) {
      if (fieldName !== "file" && fieldName !== "poster") throw new Error("Campo de archivo no permitido.");
  ...
  if (poster && (poster.mimeType !== "image/jpeg" || poster.buffer.length > 2 * 1024 * 1024)) throw new Error("El póster debe ser JPEG y no superar 2 MB.");
