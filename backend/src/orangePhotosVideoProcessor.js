...
async function register(row, metadata, derivatives, updateMetadata, possibleOrphans) {
  ...
  try {
    ...
    if(updateMetadata){
    await query(`UPDATE public.orange_photos SET duration_seconds=CASE WHEN duration_seconds IS NULL OR duration_seconds<=0 THEN $1 ELSE duration_seconds END,width=COALESCE(width,$2),height=COALESCE(height,$3),orientation=COALESCE(orientation,$4) WHERE id=$5::uuid AND family_id=$6::uuid`,[metadata.duration,metadata.width,metadata.height,metadata.rotation,row.id,row.family_id],client);
      await query(`UPDATE public.orange_photo_library_items li SET captured_at=$1::timestamptz,captured_at_source='exif' FROM public.orange_photos p WHERE p.id=li.photo_id AND p.id=$2::uuid AND p.family_id=$3::uuid AND li.user_id=p.owner_user_id AND $1::timestamptz IS NOT NULL AND li.captured_at_source IN ('upload_date','file_mtime','unknown')`,[metadata.creation_time,row.id,row.family_id],client);
    }
