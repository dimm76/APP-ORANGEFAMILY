...
function albumDateValues(body,current=null){const hasDateMode=Object.hasOwn(body,"date_mode");const mode=hasDateMode?(body.date_mode==null||body.date_mode===""?null:String(body.date_mode)):current?.date_mode??null;let start=Object.hasOwn(body,"date_start")?body.date_start||null:current?.date_start??null;let end=Object.hasOwn(body,"date_end")?body.date_end||null:current?.date_end??null;if(mode===null)return{ok:true,dateMode:null,dateStart:null,dateEnd:null};if(mode!=="single"&&mode!=="range")return{ok:false};const datePattern=/^\d{4}-\d{2}-\d{2}$/;if(!datePattern.test(String(start||"")))return{ok:false};if(mode==="single")end=start;if(mode==="range"&&(!datePattern.test(String(end||""))||String(end)<String(start)))return{ok:false};return{ok:true,dateMode:mode,dateStart:start,dateEnd:end};}
