...
La biblioteca web principal representa ahora `owned` + `library`: `owned` sigue significando propietario original y `library` una membership persistente de una foto cuyo propietario original es otro usuario. «Compartido conmigo» conserva `direct` + `album`; el visor web permite «Añadir a mi biblioteca» para una foto ajena accesible que todavía no tenga membership y «Quitar de mi biblioteca» para una foto ajena incorporada. Nunca ofrece retirar la membership del propietario original, confirma antes de quitarla, no elimina la foto original ni sus archivos y refresca la vista y el timeline tras retirarla. Android continúa sin cambios. Las acciones de propietario en el visor y en la selección masiva se ocultan para elementos ajenos, manteniendo la selección, la descarga y las operaciones compatibles.
