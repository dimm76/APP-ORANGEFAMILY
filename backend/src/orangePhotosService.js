...
function parseLibraryFilters(q,values,where){const sources=csvValues(q.access_sources),states=csvValues(q.share_states),owners=csvValues(q.owner_user_ids);if(sources.some(v=>!ACCESS_SOURCES.has(v)))return bad(400,"INVALID_FILTER","Origen de acceso no válido.");if(states.some(v=>!SHARE_STATES.has(v)))return bad(400,"INVALID_FILTER","Estado de compartición no válido.");if(owners.some(v=>!uuid(v)))return bad(400,"INVALID_FILTER","Propietario no válido.");const selectedSources=sources.length?sources:["owned","direct","album"],selectedStates=states.length?states:["private","family","selected","public_link"];where.push(`(${selectedSources.map(source=>source==="owned"?ownedAccessSql():source==="direct"?directReceivedAccessSql():albumOnlyAccessSql()).join(" OR ")})`);if(owners.length){values.push(owners);where.push(`p.owner_user_id=ANY($${values.length}::uuid[])`);}if(selectedStates.length<4){const clauses=[];if(selectedStates.includes("private"))clauses.push("p.visibility='private'");if(selectedStates.includes("family"))clauses.push("p.visibility='family'");if(selectedStates.includes("selected"))clauses.push("p.visibility='selected'");if(selectedStates.includes("public_link"))clauses.push(publicLinkActiveSql());where.push(`(${clauses.join(" OR ")})`);}return ok({owners});}
