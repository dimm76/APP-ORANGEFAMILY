...
function photoQuery(filters={},excluded=[]){const effectiveFilters=filters.album_id&&filters.access_sources_mode==="include"?{...filters,access_sources:[...new Set([...(Array.isArray(filters.access_sources)?filters.access_sources:[]),"album"])]}:filters,q=new URLSearchParams(),skip=new Set(excluded);Object.entries(effectiveFilters).forEach(([k,v])=>{if(skip.has(k)||LEGACY_FILTERS.has(k)||v==="all")return;if(Array.isArray(v)){if(v.length)q.set(k,v.join(","));}else if(v!==""&&v!=null&&v!==false)q.set(k,String(v));});return q;}
...
export const setOrangePhotoHidden=(id,isHidden)=>request(`/api/orange-photos/${encodeURIComponent(id)}/hidden`,{method:'PATCH',body:JSON.stringify({is_hidden:isHidden})});
