...
La biblioteca web principal representa ahora `owned` + `library`: `owned` sigue significando propietario original y `library` una membership persistente de una foto cuyo propietario original es otro usuario. «Compartido conmigo» conserva `direct` + `album`; Android continúa sin cambios y todavía no existe una acción frontend para crear la membership.
