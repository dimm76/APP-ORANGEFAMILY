...
function validInvitationRows(insertRow={id:invitationId,first_name:"Ana",last_name:"GarcÃ­a",email:"ana@example.com",display_name:"Ana GarcÃ­a",can_view:true,can_contribute:false,can_comment:false,status:"pending",expires_at:new Date(Date.now()+60000)}){return [{rows:[{email:"owner@example.com"}]},{rows:[]},{rows:[]},{rows:[]},{rows:[{id:"77777777-7777-4777-8777-777777777777"}]},{rows:[{id:userId,person_id:"77777777-7777-4777-8777-777777777777",email:"ana@example.com",status:"pending"}]},{rows:[]},{rows:[]},{rows:[insertRow]},{rows:[{display_name:"Owner"}]}];}
test("createInvitation exige first_name ausente, vacÃ­o o demasiado largo",async()=>{for(const first_name of [undefined,"   ","x".repeat(101)]){mockPoolResponses([{rows:[invitationAlbum]}]);const result=await guests.createInvitation(ownerReq,albumId,{first_name,last_name:"Apellidos",email:"guest@example.com"});assert.equal(result.ok,false);assert.equal(result.code,"INVALID_FIRST_NAME");assert.equal(calls.length,1);}});
test("createInvitation exige last_name ausente, vacÃ­o o demasiado largo",async()=>{for(const last_name of [undefined,"   ","x".repeat(151)]){mockPoolResponses([{rows:[invitationAlbum]}]);const result=await guests.createInvitation(ownerReq,albumId,{first_name:"Nombre",last_name,email:"guest@example.com"});assert.equal(result.ok,false);assert.equal(result.code,"INVALID_LAST_NAME");assert.equal(calls.length,1);}});
test("createInvitation conserva la validaciÃ³n existente del email",async()=>{mockPoolResponses([{rows:[invitationAlbum]}]);const result=await guests.createInvitation(ownerReq,albumId,{first_name:"Nombre",last_name:"Apellidos",email:"not-an-email"});assert.equal(result.code,"INVALID_EMAIL");assert.equal(calls.length,1);});
test("createInvitation inserta nombres normalizados y devuelve display_name",async()=>{mockPoolResponses(validInvitationRows());const result=await guests.createInvitation(ownerReq,albumId,{first_name:"  Ana ",last_name:" GarcÃ­a  ",email:" ANA@example.com "});const insert=calls.find(([sql])=>/INSERT INTO public\.orange_photo_album_guest_invitations/.test(sql));assert.match(insert[0],/invited_first_name/);assert.match(insert[0],/invited_last_name/);assert.match(insert[0],/invited_email/);assert.deepEqual(insert[1].slice(1,5),["Ana","GarcÃ­a","ana@example.com",ownerId]);assert.equal(result.payload.invitation.first_name,"Ana");assert.equal(result.payload.invitation.last_name,"GarcÃ­a");assert.equal(result.payload.invitation.display_name,"Ana GarcÃ­a");});
test("publicInvitation devuelve nombres y admite invitaciones histÃ³ricas sin nombres",async()=>{mockPoolResponses([{rows:[{first_name:"Ana",last_name:"GarcÃ­a",display_name:"Ana GarcÃ­a",email:"ana@example.com"}]}]);let result=await guests.publicInvitation("token");assert.equal(result.payload.invitation.display_name,"Ana GarcÃ­a");assert.equal(result.payload.invitation.first_name,"Ana");mockPoolResponses([{rows:[{first_name:null,last_name:null,display_name:null,email:"old@example.com"}]}]);result=await guests.publicInvitation("old-token");assert.equal(result.payload.invitation.display_name,null);assert.equal(result.payload.invitation.email,"old@example.com");});
