...
function handleOrangePhotosRoutes(app) {
  ...
  app.get("/api/public/orangephotos/album/:token/photos/:photoId/url", safe(async req => { const result=await service.publicAlbumPhoto(req.params.token,req.params.photoId); return result.ok?{ok:true,payload:{url:result.payload.item.original_url,expires_in:3600}}:result; }, "No se pudo preparar la URL pública."));
  app.get("/api/public/orangephotos/album/:token/photos/:photoId/download", async (req,res) => { try { const result=await service.publicAlbumPhoto(req.params.token,req.params.photoId); if(!result.ok)return send(res,result); const file=(await require("../db").query(`SELECT bucket,object_key,mime_type,size_bytes FROM public.orange_photo_files WHERE photo_id=$1 AND variant='original' LIMIT 1`,[result.payload.raw.id])).rows[0]; if(!file)return send(res,{ok:false,status:404,code:"PUBLIC_NOT_FOUND",reason:"Recurso público no encontrado."});const download=await service.downloadObject(file),clean=attachmentName(result.payload.raw.original_filename);res.setHeader("Content-Type",download.ContentType||"application/octet-stream");res.setHeader("Content-Disposition",`attachment; filename="${clean}"; filename*=UTF-8''${encodeURIComponent(clean)}`);return download.Body.pipe(res); } catch { return res.status(404).json({ok:false,code:"PUBLIC_NOT_FOUND",message:"Recurso público no encontrado.",details:null}); } });
