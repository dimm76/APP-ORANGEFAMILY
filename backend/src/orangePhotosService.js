...
async function addPhoto(req,albumId,body,remove=false){const a=await albumOwned(req,albumId);if(!a.ok)return a;const photoId=body.photo_id||body.photoId;if(!uuid(photoId))return bad(400,"Foto no válida.");const d=await detail(req,photoId);if(!d.ok)return d;const c=await pool.connect();try{await c.query('BEGIN');let changed;if(remove)changed=(await c.query(`DELETE FROM public.orange_photo_album_items WHERE album_id=$1 AND photo_id=$2 RETURNING photo_id`,[albumId,photoId])).rowCount;else changed=(await c.query(`INSERT INTO public.orange_photo_album_items(album_id,photo_id,sort_order,added_by) VALUES($1,$2,$3,$4) ON CONFLICT(album_id,photo_id) DO NOTHING RETURNING photo_id`,[albumId,photoId,Number(body.sort_order)||0,a.payload.auth.userId])).rowCount;if(changed&&!remove&&d.payload.item.media_type==='image')await c.query(`UPDATE public.orange_photo_albums SET cover_photo_id=$1,updated_by=$2 WHERE id=$3 AND cover_photo_id IS NULL`,[photoId,a.payload.auth.userId,albumId]);if(changed)await recordPhotoEvent(c,{familyId:a.payload.auth.familyId,photoId,actorUserId:a.payload.auth.userId,eventType:remove?'removed_from_album':'added_to_album',...clientContext(req),metadata:{album_id:albumId}});await c.query('COMMIT');return ok(remove?{removed:Boolean(changed)}:{added:Boolean(changed)});}catch(error){await c.query('ROLLBACK');throw error;}finally{c.release();}}
