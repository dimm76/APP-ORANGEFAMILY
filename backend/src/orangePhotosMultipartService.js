...
async function complete(req,id,body={}){
  ...
  let result;try{result=await photos.insertPhoto(found.payload.auth,metadata,{provider:upload.provider,bucket:upload.bucket,object_key:upload.object_key,mime_type:upload.mime_type,size_bytes:Number(upload.size_bytes),checksum_sha256:checksum,etag:head.etag},null,null,config.force_duplicate&&suppression?checksum:null);}
